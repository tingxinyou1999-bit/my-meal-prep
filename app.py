...
with tab3:
    st.markdown("### 👨‍🍳 科学备餐实验室 (Meal Prep Lab)")
    st.info("💡 **高效原则**：一次采购，三次处理，五分钟出餐。拒绝繁琐，专注营养。")
    ...
    with col_step1:
        with st.expander("📦 存储与保鲜黑科技 (点击展开)", expanded=True):
            st.write("""
            1.  **冷却再盖盖**：防止水汽让叶菜变黄变烂。
            2.  **垫纸巾大法**：餐盒底部垫一张厨房纸，多放 2 天也新鲜。
            3.  **生熟分离**：肉类分袋装好冷冻，吃前现煎口感最好。
            """)

    with col_step2:
        with st.expander("🚨 身体信号与调整策略", expanded=True):
            st.write("""
            * **感到脑雾/疲劳**：补充 10 颗无盐杏仁或大量饮水。
            * **全天饮水指标**：确保达到 3000ml (Johor 炎热天气必选)。
            * **运动建议**：配合轻度运动，BMI 达标更迅速。
            """)

    st.success("✨ **营养师寄语**：减脂不是苦行僧，而是学会与身体对话。祝你顺利通往 70kg！")
