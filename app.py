...
with st.sidebar.expander("👤 输入身体指标 (必填)", expanded=True):
    weight = st.sidebar.number_input("当前体重 (kg)", 40.0, 150.0, 80.0)
    height = st.sidebar.number_input("当前身高 (cm)", 120.0, 220.0, 175.0)
    age = st.sidebar.number_input("当前年龄", 15, 85, 25)
    gender = st.sidebar.selectbox("性别", ["男", "女"])
    activity = st.sidebar.selectbox("活动水平", [
        "久坐 (办公室/交易员)", 
        "轻度活动 (每周运动1-2天)", 
        "中度活动 (每周运动3-5天)", 
        "高度活动 (每天高强度运动)"
    ])
    user_goal = st.sidebar.selectbox("核心目标", ["维持体重", "减脂 (Cut)", "增肌 (Bulk)"], index=1)
...
st.info(f"📍 目标状态：{user_goal} | 建议摄入：{int(target_cal)} kcal")
...
with tab2:
    ...
    if st.button("🪄 一键生成动态 5 天方案"):
        ...
        st.markdown(f'<div style="text-align: center;"><a href="https://wa.me/?text={encoded_text}" target="_blank" class="whatsapp-button">📲 发送清单到 WhatsApp</a></div>', unsafe_allow_html=True)
